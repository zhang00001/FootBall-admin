  ...
  <div class="index-vue">
    ...
    <aside :class="asideClassName">
      ...
      <div class="logo-c">
        ...
        <span v-show="isShowAsideTitle">大神推球</span>
      </div>
      <!-- 菜单栏 -->
      <Menu
        ref="asideMenu"
      ...
      >
        ...
        <div v-for="(item, index) in menuItems" :key="index">
          <template v-if="item.mate.isShow">
            <template v-if="user[item.mate.power]==1||item.mate.power==1">
    ...
    <Modal v-model="modal1" title="登录已过期" @on-ok="ok" @on-cancel="cancel">
      <p>请重新登录</p>
    </Modal>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    menuItems() {
      //      this.user=JSON.parse(localStorage.getItem('user'))
      //      console.log(JSON.parse(JSON.stringify(this.$store.state.menuItems)) )
      //      let menus=JSON.parse(JSON.stringify(this.$store.state.menuItems))

      // let a=menus.filter(val=>{
      //     val.mate.isShow==true
